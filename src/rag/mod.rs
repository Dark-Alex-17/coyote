...
impl Rag {
    ...
    pub async fn attach(
        ...
        write_rag_sbx_mixin(save_path, &host, name, driver, api_key_entry.is_some())?;
...
fn generate_rag_sbx_mixin(
    ...
    let credentials = api_key_env
        .map(|env_var| mcp_credentials::CredentialEntry {
            ...
            description: format!("API key for the RAG '{service_name}'"),
    ...
    let content = mcp_credentials::render_mixin_document(
        ...
        &format!(
            "Auto-generated by Coyote for RAG '{service_name}'. Allows \
...
/// Writes the sandbox sidecar for a RAG whose store lives on a remote host.
///
/// Both creation paths need the identical sidecar: attaching to a collection
/// someone else filled and creating one Coyote owns reach the same host over
/// the same protocol with the same credential. Ownership of the documents
/// changes nothing the sandbox can see. Routing both through one call site is
/// what stops them from drifting — the owned path was silently missing the
/// sidecar, which is invisible on the host and fails every query inside sbx.
fn write_rag_sbx_mixin(
    rag_yaml_path: &Path,
    host: &str,
    name: &str,
    driver: &str,
    ...
    let env_var = has_api_key.then(|| rag_env_var_name(name));
...
mod tests {
    ...
    /// Renders through `write_rag_sbx_mixin` — the entry point both the attach
    /// and the creation wizard call — rather than through the raw generator, so
    /// a divergence between the two would surface here.
    fn render_via_wizard_entry_point(name: &str, has_api_key: bool) -> serde_yaml::Value {
        let dir = TempDir::new("owned-mixin");
        let yaml_path = dir.path.join(format!("{name}.yaml"));
        ...
        let text = fs::read_to_string(dir.path.join(format!("{name}.sbx-mixin.yaml")))
            .expect("no sidecar means the host is never allow-listed inside sbx");
        serde_yaml::from_str(&text).unwrap()
    }

    /// An owned Qdrant RAG is `attached: false`, but it dials the same remote
    /// host with the same credential as an attached one, and without this file
    /// every query inside sbx fails — on the host it keeps working, so nothing
    /// else would catch it. Parity with the attached case is structural (one
    /// call site, and no parameter distinguishes them), so what is asserted
    /// here is the content itself.
    #[test]
    fn an_owned_qdrant_rag_gets_a_sidecar_naming_its_host_and_key() {
        let parsed = render_via_wizard_entry_point("owned-docs", true);

        assert_eq!(parsed["name"].as_str(), Some("rag-owned-docs"));
        assert_eq!(allow_list(&parsed), vec!["qdrant.example.com:6333"]);

        let credential = &parsed["credentials"][0];
        assert_eq!(credential["service"].as_str(), Some("owned-docs"));
        assert_eq!(
            credential["apiKey"]["name"].as_str(),
            Some("OWNED_DOCS_API_KEY"),
            "the env var must be the one the host binds under the same service id"
        );

        let inject = &credential["apiKey"]["inject"][0];
        assert_eq!(inject["domain"].as_str(), Some("qdrant.example.com:6333"));
        assert_eq!(inject["header"].as_str(), Some("api-key"));
        assert_eq!(inject["format"].as_str(), Some("%s"));
    }

    /// A Qdrant instance without authentication still needs its host allowed;
    /// a missing key must not cost the RAG its network access.
    #[test]
    fn an_owned_qdrant_rag_without_a_key_still_gets_its_host_allowed() {
        let parsed = render_via_wizard_entry_point("keyless-docs", false);

        assert_eq!(allow_list(&parsed), vec!["qdrant.example.com:6333"]);
        assert!(parsed["credentials"].is_null());
    }
