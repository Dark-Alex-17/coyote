...
/// Replaces or inserts the entry of every degraded/failed provider from the
/// embedded catalog, preserving the fixed provider order of
/// [`PROVIDER_SOURCES`]. A synced override must never carry fewer models for a
/// provider than the embedded catalog: an upstream outage would otherwise
/// shrink the model list until the next successful sync.
pub fn backfill_embedded_floor(build: &mut CatalogBuild, embedded: &[ProviderModels]) {
    let needs_backfill: HashSet<&str> = build
        .degraded
        .iter()
        .chain(build.failed.iter())
        .map(String::as_str)
        .collect();
    if needs_backfill.is_empty() {
        return;
    }
    let mut providers = Vec::with_capacity(PROVIDER_SOURCES.len());
    for source in &PROVIDER_SOURCES {
        let built = build
            .providers
            .iter()
            .position(|provider| provider.provider == source.provider)
            .map(|position| build.providers.remove(position));
        if needs_backfill.contains(source.provider)
...
pub async fn generate_models_file(path: &Path) -> Result<()> {
    ...
    if sources.modelsdev.is_none() || sources.openrouter.is_none() {
        bail!(
            "generating the models file requires both catalog sources; see the log for the failed fetch"
        );
    }
...
"#,
    ...
    fn embedded_provider<'a>(providers: &'a [ProviderModels], name: &str) -> &'a ProviderModels {
        providers
            ...
            .unwrap_or_else(|| panic!("provider '{name}' missing from models.yaml"))
    }

    #[test]
    fn embedded_catalog_invariants() {
        let providers = embedded_catalog();
        assert!(providers.len() >= 24);
        let order: Vec<&str> = providers
            .iter()
            .map(|provider| provider.provider.as_str())
            .collect();
        ...
        assert!(embedded_provider(&providers, "xai").oauth.is_some());
        for name in ["jina", "voyageai"] {
            assert!(
                embedded_provider(&providers, name)
                    .models
                    .iter()
                    .any(|model| model.model_type == "embedding"),
                "{name} lost its embedding models"
            );
        }
        assert!(
            embedded_provider(&providers, "cohere")
                .models
                .iter()
                .any(|model| model.model_type == "reranker"),
            "cohere lost its reranker models"
        );

        for provider in &providers {
            for model in &provider.models {
                // A :thinking model without a patch must at least alias a real
                // wire name (qianwen twins invert a base-model patch instead).
                if model.name.ends_with(":thinking") {
                    assert!(
                        model.patch.is_some() || model.real_name.is_some(),
                        "{}:{} has neither patch nor real_name",
                        provider.provider,
                        model.name
                    );
                }
                if model.model_type == "chat" {
                    let serialized = serde_yaml::to_value(model).unwrap();
                    assert!(
                        serialized
                            .as_mapping()
                            .unwrap()
                            .keys()
                            .any(|key| !matches!(key.as_str(), Some("name" | "type"))),
                        "{}:{} is an all-default chat entry",
                        provider.provider,
                        model.name
                    );
                }
        ...
        for model in &embedded_provider(&providers, "claude").models {
            if model.model_type == "chat" {
                assert!(
                    model.require_max_tokens,
                    "claude:{} is missing require_max_tokens",
                    model.name
                );
            }
        }
    ...
    #[test]
    fn embedded_catalog_roundtrip() {
        let providers = embedded_catalog();
        let yaml = serde_yaml::to_string(&providers).unwrap();
        let reparsed: Vec<ProviderModels> = serde_yaml::from_str(&yaml).unwrap();
        assert_eq!(reparsed.len(), providers.len());
        for (reparsed, original) in reparsed.iter().zip(&providers) {
            assert_eq!(reparsed.provider, original.provider);
            assert_eq!(reparsed.models.len(), original.models.len());
        }
    }

    fn provider_entry(provider: &str, model_names: &[&str]) -> ProviderModels {
        ProviderModels {
            provider: provider.to_string(),
            oauth: None,
            models: model_names
                .iter()
                .map(|name| ModelData::new(name))
                .collect(),
        }
    }

    #[test]
    fn backfill_replaces_degraded_and_inserts_failed_providers() {
        let mut build = CatalogBuild {
            providers: vec![
                provider_entry("openai", &["gpt-fresh"]),
                provider_entry("gemini", &["quirks-only"]),
            ],
            degraded: vec!["gemini".to_string()],
            failed: vec!["claude".to_string()],
        };
        let embedded = vec![
            provider_entry("openai", &["gpt-old-1", "gpt-old-2"]),
            provider_entry("gemini", &["gem-1", "gem-2", "gem-3"]),
            provider_entry("claude", &["claude-1"]),
        ];

        backfill_embedded_floor(&mut build, &embedded);

        let order: Vec<&str> = build
            .providers
            .iter()
            .map(|provider| provider.provider.as_str())
            .collect();
        assert_eq!(order, ["openai", "gemini", "claude"]);
        assert_eq!(find_provider(&build, "openai").models.len(), 1);
        assert_eq!(find_provider(&build, "gemini").models.len(), 3);
        assert_eq!(find_provider(&build, "claude").models.len(), 1);
        assert_eq!(
            find_provider(&build, "openai").models[0].name,
            "gpt-fresh",
            "providers with live API data must not be touched"
        );
    }

    #[test]
    fn backfill_keeps_built_entry_when_embedded_lacks_the_provider() {
        let mut build = CatalogBuild {
            providers: vec![provider_entry("gemini", &["quirks-only"])],
            degraded: vec!["gemini".to_string()],
            failed: vec![],
        };

        backfill_embedded_floor(&mut build, &[]);

        assert_eq!(find_provider(&build, "gemini").models.len(), 1);
    }

    #[test]
    fn backfill_without_degraded_or_failed_is_a_no_op() {
        let mut build = CatalogBuild {
            providers: vec![provider_entry("openai", &["gpt-fresh"])],
            degraded: vec![],
            failed: vec![],
        };

        backfill_embedded_floor(&mut build, &[provider_entry("openai", &["a", "b"])]);
    ...
    fn backfill_restores_embedded_floor_for_unresolved_providers() {
        ...
        for provider in unresolved {
            let built = find_provider(&build, &provider);
            let floor = embedded
                .iter()
                .find(|embedded| embedded.provider == provider)
                .unwrap();
            assert!(
                built.models.len() >= floor.models.len(),
                "provider '{provider}' shrank below the embedded floor"
            );
        }
    }
